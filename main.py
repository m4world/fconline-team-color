...
app = FastAPI(title="FC Online 랭커 분석")
...
def batch_fetch_and_store(matchtype=52):
...
def get_overview():
    ...
    top_players = conn.execute("SELECT spid, spname, sp_position, goal, assist, matchCount FROM player_stats ORDER BY matchCount DESC LIMIT 50").fetchall()
    conn.close()
    return {
        "total_players": total,
...
@app.post("/api/fetch")
def trigger_fetch(matchtype: int = 50): # 기본값을 공식경기(50)로 설정
